...
const command = {
  data: new SlashCommandBuilder()
    .setName("weeklyrecap")
    .setDescription("Manually run this week's matchup recaps into #weekly-reports")
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
  async execute(interaction) {
    await interaction.reply({ content: "Running weekly recap...", ephemeral: true });
    try {
      const { week, matchupCount } = await postWeeklyRecap(interaction.client, interaction.guildId);
      await interaction.editReply(`Posted week ${week} recap (${matchupCount} matchups).`);
    } catch (err) {
