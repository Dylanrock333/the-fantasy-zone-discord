...
const { getMatchupPreview } = require("../utils/fantasyBotClient");
...
async function getLastRecapSummary(client, config) {
  ...
  const messages = await channel.messages.fetch({ limit: settings.recap.contextScanLimit });
...
async function postMatchupPreview(client, guildId) {
  const config = getGuildConfig(guildId, "matchupChannelId");
...
async function postMatchupPreviewForAllGuilds(client) {
  await forEachGuild(client, postMatchupPreview, "Matchup preview");
}
