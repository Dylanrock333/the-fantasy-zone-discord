...
const handler = {
  customId: "tradeCompare:teamSelectA",
  async execute(interaction) {
    ...
    try {
      ...
      updateSession(interaction.message.id, { teamA: { id, name }, rosterA: players, selectedA: [] });
      await interaction.update(renderPanel(session));
    } catch (err) {
      logger.error("trade compare: failed to load team A roster:", err);
